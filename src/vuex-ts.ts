...
export interface ModuleOptions<
  ...
  actions?: Actions 
  & DefaultActions & ThisType<ReadonlyTree<State> & ReadonlyTree<Getters> & Mutations & MutationsAsync & Actions & Modules & { $store: Store<any>, commitAsync: Async }>;
}
...
//
  ...
  actions: {
    async actionA(x: number, y: number) {
      return {x, y};
    },

    actionB(x: number, y: number): Promise<{ x: number, y: number }> {
      const result = this.commitAsync(this.asyncMutationB, Promise.resolve("result of the promise"), ["asdf", 3])
      console.log(result);
      const r = this.$store.dispatch('rootModule/actionA', [x, y])
      return r;
    }
  }
});
...
var scopedStore = createStore({
  ...
  actions: {
    async actionA() {
      ...
      const result = await this.commitAsync(this.asyncMutationB, Promise.resolve("result of the promise"), ["asdf", 3])
      console.log(result);
...
const result = rootModule.actionB(5, 7);
